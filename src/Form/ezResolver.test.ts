...
describe('ezResolver', () => {
  ...
  it('reports required on every empty value, with the label in the default message', async () => {
    for (const empty of [undefined, null, '', false, []]) {
      const result = await run(
        z.object({ tos: z.unknown() }),
        { tos: empty },
        { tos: rules({ required: true }, 'Terms') },
      )
  ...
  it('runs validate even when the value is empty or false, like hookform', async () => {
    ...
    expect((await run(schema, { v: false }, { v: optIn })).errors).toEqual({
      v: { type: 'validate', message: 'You must opt in' },
    })
    expect((await run(schema, { v: '' }, { v: optIn })).errors).toEqual({
      v: { type: 'validate', message: 'You must opt in' },
    })
  ...
  it('fails required on false and on empty, but skips minLength on an empty string', async () => {
    const schema = z.object({ v: z.unknown() })
    expect(
      (await run(schema, { v: false }, { v: rules({ required: true }, 'Terms') })).errors,
    ).toEqual({
  ...
  it('skips the other rules when the value is empty and not required', async () => {
    const result = await run(
      text,
      { nick: '' },
      { nick: rules({ minLength: 3, pattern: /^x$/ }, 'Nickname') },
    )
    expect(result.errors).toEqual({})
  })
  ...
  it('checks rules in hookform order and reports the first failure', async () => {
    const result = await run(
      text,
      { nick: 'abc' },
      { nick: rules({ maxLength: 2, pattern: /^\d+$/ }, 'Nickname') },
    )
    expect(result.errors).toEqual({
      nick: { type: 'maxLength', message: 'Nickname must be at most 2 characters.' },
    })
  })
  ...
  it('compares min/max numerically when both sides are numbers, else as strings', async () => {
    ...
    const age = (value: string, r: FieldRules) =>
      run(schema, { age: value, day: '' }, { age: rules(r, 'Age') })
    expect((await age('5', { min: 18 })).errors).toEqual({
      age: { type: 'min', message: 'Age must be at least 18.' },
    })
    expect(
      (await age('100', { max: { value: 99, message: 'Nobody is that old' } })).errors,
    ).toEqual({
    ...
    const day = await run(
      schema,
      { age: '', day: '2020-01-01' },
      { day: rules({ min: '2021-01-01' }, 'Day') },
    )
    expect(day.errors).toEqual({
      day: { type: 'min', message: 'Day must be at least 2021-01-01.' },
    })
  })
  ...
  it('derives minLength/maxLength messages from the label and honours overrides', async () => {
    ...
    expect(short.errors).toEqual({
      nick: { type: 'minLength', message: 'Nickname must be at least 3 characters.' },
    })
    const long = await run(
      text,
      { nick: 'abcdefghijklmnop' },
      { nick: rules({ maxLength: { value: 12, message: 'Too long!' } }) },
    )
  ...
  it('resets lastIndex on global regexes so a pattern keeps failing on repeated runs', async () => {
    ...
    for (let i = 0; i < 3; i++) {
      expect((await run(text, { nick: 'ABC' }, fields)).errors).toEqual({
        nick: { type: 'pattern', message: 'Nickname is invalid.' },
      })
    }
    expect((await run(text, { nick: 'abc' }, fields)).errors).toEqual({})
  })
  ...
  it('runs validate functions: string is the message, false uses the default, true/undefined pass, async is awaited', async () => {
    ...
    const check = (value: string, r: FieldRules<string>) =>
      run(user, { user: value }, { user: rules(r, 'Username') })
    expect((await check('admin', { validate: (v) => v !== 'admin' || 'Reserved' })).errors).toEqual(
      {
        user: { type: 'validate', message: 'Reserved' },
      },
    )
    expect((await check('x', { validate: () => false })).errors).toEqual({
      user: { type: 'validate', message: 'Username is invalid.' },
    })
    expect((await check('x', { validate: () => true })).errors).toEqual({})
    expect((await check('x', { validate: () => undefined })).errors).toEqual({})
    expect((await check('x', { validate: async () => 'Later' })).errors).toEqual({
      user: { type: 'validate', message: 'Later' },
    })
  })
  ...
  it('hands validate the whole form values', async () => {
    ...
    const result = await run(
      schema,
      { a: '1', b: '2' },
      { b: rules({ validate: (v, all) => v === all.a || 'Must match a' }) },
    )
  ...
  it("keeps zod's error when no rule fails", async () => {
    const result = await run(
      email,
      { email: 'nope' },
      { email: rules({ required: true }, 'Email') },
    )
  ...
  it('only checks rules for the names being validated but still returns the full errors object', async () => {
    ...
    const fields = {
      email: rules({ required: true }, 'Email'),
      nick: rules({ required: true }, 'Nickname'),
    }
  ...
  it('reads nested field rules by path', async () => {
    ...
    const result = await run(
      schema,
      { address: { street: '' } },
      { 'address.street': rules({ required: true }, 'Street') },
    )
    expect(result.errors).toEqual({
  ...
  it('applies minLength/maxLength/pattern to string values only, like hookform', async () => {
    const schema = z.object({ v: z.unknown() })
    const check = (value: unknown, r: FieldRules) =>
    ...
    expect((await check('123', { pattern: /^[a-z]+$/ })).errors).toEqual({
      v: { type: 'pattern', message: 'Value is invalid.' },
    })
    expect((await check('ab', { minLength: 3 })).errors).toEqual({
      v: { type: 'minLength', message: 'Value must be at least 3 characters.' },
    })
    expect((await check('abc', { maxLength: 2 })).errors).toEqual({
      v: { type: 'maxLength', message: 'Value must be at most 2 characters.' },
    })
  })
  ...
  it('skips fields that are not mounted', async () => {
    const result = await run(
      text,
      { nick: '' },
      { nick: { ...rules({ required: true }, 'Nickname'), mount: false } },
    )
    expect(result.errors).toEqual({})
  })
  ...
  it("accepts bare hookform rules from a consumer's own useController", async () => {
    ...
    expect(result.errors).toEqual({
      nick: { type: 'required', message: 'This field is required.' },
    })
  })
})
