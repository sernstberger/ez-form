...
import { useRegisterErrorSummary } from './ErrorSummaryContext'
 ...
 * validation attempt, GOV.UK-style: a `role="alert"` region focused on failure, with each item
 * a link that focuses its field. Placed by the consumer — under the form title, or inside the
...
export function FormErrorSummary(inProps: FormErrorSummaryProps) {
  ...
  // Outside a wizard: any failed submit (submitCount > 0) with errors left standing. Inside a
  // wizard: a failed Next/go against the current step (wizard.lastFailed is non-null) — a
  // wizard's own submitCount includes failures on other steps' fields, which this step never
  // showed and must not surface as its own summary.
  const attempted = wizard ? wizard.lastFailed != null : submitCount > 0
  const visible = attempted && entries.length > 0

  const headingRef = useRef<HTMLElement>(null)
  // Re-focus on each new failed attempt: a wizard step's own `lastFailed` reference changes
  // (a fresh array) on every failed validation of that step, and a plain form's `submitCount`
  // increments on every submit — both are stable between re-renders that aren't a new attempt,
  // so the effect fires only when there is something new to announce.
  const attemptKey = wizard ? wizard.lastFailed : submitCount
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      >
        {entries.map((entry) => {
          const fieldId = fieldElementId(entry.name)
          return (
