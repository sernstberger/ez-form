...
- [ ] Interactive targets ≥ 24×24 px (`expectTargetSize`). An exemption is allowed only when the size is the library's, the consumer opted into it, and the test pins the measured number with the ruling — the audit note in `src/test/targetSize.ts` lists every one (today: `Rating size="small"`, 18×18, #111).
