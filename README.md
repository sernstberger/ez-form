...
| `Form` (label placement)                       | —                                             | `labelPlacement?: 'floating' \| 'stacked' \| 'start'` (default `'floating'`), `labelPlacementBreakpoint?` (default `'sm'`), `labelWidth?` (default `'12rem'`) — see "Label placement" below                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
**Label placement.** `Form`'s `labelPlacement` decides where every field's label sits relative to its control. It is its **own axis**, deliberately not a fourth MUI `variant`: `TextFieldVariants` is a closed union (`'outlined' | 'standard' | 'filled'`) with no `*PropsVariantOverrides` interface to augment, and the runtime picks the input component from a fixed map, so a fourth value neither typechecks nor renders. `variant` picks the box; `labelPlacement` picks where the label goes, and the two compose — `<TextField variant="filled">` under `labelPlacement="start"` is a filled box with its label in a left column.

| Value                  | What it looks like                                                                                                                                                                                                                                         |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `'floating'` (default) | MUI's own: the label sits over the input and translates up on focus or fill, notching the outline.                                                                                                                                                         |
| `'stacked'`            | The label sits above the control in normal flow — no motion, no notch, helper text flush left. What `createEzFormTheme()` sets, and what `DESIGN.md` calls for.                                                                                            |
| `'start'`              | The label sits in a column beside the control (the settings-form layout), collapsing back to `'stacked'` below `labelPlacementBreakpoint` (default `'sm'`) because a phone has no room for two columns. `labelWidth` (default `'12rem'`) sizes the column. |
...
| `labelAs`          | The frame renders                                                                                      | Use it for                                        |
| ------------------ | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------- |
| `'none'` (default) | No label element. `bound.displayLabel`, `labelId`, `controlId` and `labelRequired` are yours to place. | A control that owns its own label — most of them. |
| `'control'`        | MUI `FormControlLabel`: a `<label>` wrapping your control, label text beside it.                       | A single checkbox- or switch-shaped input.        |
| `'legend'`         | `<fieldset>` + `<legend>`. Point your group at `bound.labelId` with `aria-labelledby`.                 | A group of controls under one name.               |
