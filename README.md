...
| `Slider`                                       | MUI `Slider`                                  | `name`, `label` (legend), `helperText?`; rules `min`, `max` (also the slider bounds), `validate` — no `required`, since a slider always reports a value. Value is a `number`, or `[number, number]` for a range. For a range, name each thumb with MUI's `getAriaLabel` **plus** `slotProps={{ input: { 'aria-labelledby': undefined } }}` — the legend reference otherwise outranks it and both thumbs share one name                                                                                                                                                                                        |
