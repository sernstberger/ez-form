...
| `TextField`                                    | MUI `TextField`                               | `name`, `variant?: EzTextFieldVariants` (MUI's three, plus anything augmented — see [Custom variants](#custom-variants)); rules `required`, `min`, `max`, `minLength`, `maxLength`, `pattern`, `validate`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `Select`                                       | MUI `TextField select`                        | `name`, `options: readonly SelectOption[]` (`{ value: string \| number; label: string }`), `variant?: EzTextFieldVariants` (inherited from `TextFieldProps` — see [Custom variants](#custom-variants)); the same rules as TextField, typed over the option value                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `Autocomplete`                                 | MUI `Autocomplete`                            | `name`, `options`, `getOptionValue?` (default `o => o.value`; return `o` to store objects), `multiple`, `freeSolo`, `textFieldProps?` (takes `variant?: EzTextFieldVariants`); all TextField rules. Options may carry extra fields (they reach `onChange`)                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `NumberField`                                  | Base UI `NumberField` through MUI `TextField` | `name`, `label?`, `helperText?`, `size?`, `variant?: EzTextFieldVariants`; rules `required`, `min`, `max` (also the stepper bounds), `validate`. Value is `number \| null`; digits group while typing (new in v2.1), and `format={{ useGrouping: false }}` turns that off                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `DatePicker` / `TimePicker` / `DateTimePicker` | MUI X pickers                                 | `name`, `label?`, `helperText?`, `errorMessages?`; rules `required`, `validate`. The picker's own props (`minDate`, `disablePast`, `views`, …) pass through; `slotProps.textField.variant` keeps MUI X's own closed union (`'outlined' \| 'standard' \| 'filled'`) — the pickers get the static label from the preset's `MuiPickersTextField.defaultProps`, not from `EzTextFieldVariants`, see [Custom variants](#custom-variants). Value is the adapter's date type or `null`                                                                                                                                                                                                                                      |
...
**Floating is `InputLabel`'s, not this axis's — and under the preset it is the `variant`'s.** Whether a `top` label sits over the input and translates up on focus is MUI's `InputLabel` `shrink` (with `disableAnimation` and `OutlinedInput` `notched`), which is a theme concern in vanilla MUI and stays one here. Under stock `createTheme()` a `top` label floats whatever the variant. Under `createEzFormTheme()` the answer is the field's `variant`: `variant="stacked"` — the preset's `MuiTextField` default — is static, above the control in normal flow, with helper text flush left, which is what `DESIGN.md` calls for; `variant="outlined"` on one field restores MUI's floating label. See [Custom variants](#custom-variants). A consumer on a stock theme who wants static labels writes the same three theme lines a vanilla MUI consumer writes today.
...
MUI's `TextFieldVariants` is a closed union (`'outlined' | 'standard' | 'filled'`) with no `TextFieldPropsVariantOverrides` interface to augment, and `TextField` throws at render for a value outside its fixed input map. ez-form ships the fix locally, **under the upstream names**: declare the variant with the same `declare module` line the MUI docs will show once upstream lands, style it with `theme.components.MuiTextField.variants`, and use it.
...
A custom variant renders MUI's `OutlinedInput`, matching the upstream patch's fallback. `TextField` only passes `label` to the input under `'outlined'`, so the notch never opens — the box is a plain outlined root for the theme's `variants` to dress. Pass `slots.input` for another input, as upstream. `variant` reaches `TextField`, `NumberField`, `Autocomplete`'s `textFieldProps`, and — with no code of its own — `Select`, since it inherits `TextFieldProps` and MUI's `TextField` hands `Select` the same input.

`'stacked'` is the variant ez-form itself declares, and `createEzFormTheme()` both styles it (the static label — in normal flow, no transform, no animation) and sets it as `MuiTextField`'s default. The preset also sets `MuiTextField.defaultProps.slots.input` to a small switching slot (`VariantInput`) that reads the resolved `variant` from `FormControl` context and renders MUI's own `Input`, `FilledInput` or `OutlinedInput` for the built-ins and `OutlinedInput` for anything else — because with `variant` defaulted to `'stacked'` a consumer's own bare `<MuiTextField>` under the preset would otherwise have no input to render (MUI's `variantComponent` map has no `'stacked'` entry) and crash, and a constant `OutlinedInput` would have hijacked a bare `variant="filled"`. The pickers get the same through `MuiPickersTextField.defaultProps` (`PickersVariantInput`). So under the preset every field is `variant="stacked"`, and `variant="outlined"` on one field opts that field back to MUI's floating label. Under a stock `createTheme()` `'stacked'` typechecks and renders, but it is an outlined box with a floating label and no notch — the look is the preset's, not the type's.

**Known limitation.** The preset's _box_ rules (root border, focus ring, padding) stay theme-wide in this pass, so under the preset `variant="outlined"` floats its label over the root's solid border. Follow-up #143 ("Preset: a real notched look for `variant="outlined"`") keys the box rules on the variant.

The shim is temporary. Every line of it carries the marker `UPSTREAM SHIM (#142)`, so when `@mui/material` exports `TextFieldPropsVariantOverrides` itself the deletion is one grep and consumers change nothing — their `declare module` block then augments MUI's own interface, and `EzTextFieldVariants` becomes an alias of `TextFieldVariants`.

The pickers (`DatePicker`/`TimePicker`/`DateTimePicker`) are theme-only: they do not take `EzTextFieldVariants` on `slotProps.textField.variant`, because MUI X's `PickersTextFieldProps` is a discriminated union and re-declaring it would be the re-implementation the project's philosophy forbids for a prop that already has a working channel. Instead the preset sets `MuiPickersTextField.defaultProps = { variant: 'stacked', slots: { input: PickersOutlinedInput } }`, so every picker gets the static label with no per-field plumbing; a consumer opts one picker back to MUI's floating label with `slotProps={{ textField: { variant: 'outlined' } }}`, MUI X's own type.

`EzTextFieldVariants` is exported from the package root for typing your own `variant` props; the internal helpers (`customVariantSlots`, `isBuiltInTextFieldVariant`) are not exported — reach the same behavior through `slots.input`, as shown above.
