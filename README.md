...
> **Zod gotcha:** by default, zod skips a `.superRefine()` entirely once anything else in the same object produced a "non-continuable" issue — a `z.enum`/`z.literal` mismatch, among others — even in a completely unrelated field on a completely unrelated step. A blank `purpose` `Select` three steps away can silently swallow your `superRefine`'s messages. Pass `{ when: () => true }` as the second argument to make the refinement run regardless of what else in the object failed: `.superRefine(fn, { when: () => true })`. Every `superRefine` in `ConditionalFields`, `Checkout`, `SignUp`, and `Loan` does this for exactly that reason.
