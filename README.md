...
A step with no `when` is always shown, at no cost: `Wizard` only subscribes to live form values (`useWatch`) when at least one step defines `when`. A hidden step disappears from the stepper and from `next`/`prev`/`go` navigation and `page` layout, and its `fields` are skipped by Next; if it was the current step, the wizard moves to the nearest visible step before it (or the first, if none qualify). `visited` still remembers a hidden step's id, so it comes back as completed — not upcoming — if it becomes visible again, and a saved `visited`/`step` that names a currently-hidden step resumes on the nearest visible one instead.

`when` only controls navigation and per-step validation — final submit still validates the whole schema, so a hidden step's fields need to be genuinely optional at the schema level (a `superRefine` that only requires them when the gate is true, or a discriminated union) the same way a field listed in no step's `fields` already does.
