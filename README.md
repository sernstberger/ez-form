...
**Label placement.** `Form`'s `labelPlacement` decides **where** every field's label sits relative to its control. It takes MUI `FormControlLabel`'s own vocabulary, `'top' | 'start'`, and it is its **own axis**, deliberately not a fourth MUI `variant`: `TextFieldVariants` is a closed union (`'outlined' | 'standard' | 'filled'`) with no `*PropsVariantOverrides` interface to augment, and the runtime picks the input component from a fixed map, so a fourth value neither typechecks nor renders. `variant` picks the box; `labelPlacement` picks where the label goes, and the two compose — `<TextField variant="filled">` under `labelPlacement="start"` is a filled box with its label in a left column.
...
A custom variant renders MUI's `OutlinedInput`, matching the upstream patch's fallback. `TextField` only passes `label` to the input under `'outlined'`, so the notch never opens — the box is a plain outlined root for the theme's `variants` to dress. Pass `slots.input` for another input, as upstream.

`'stacked'` is the variant ez-form itself declares, and `createEzFormTheme()` both styles it (the static label — in normal flow, no transform, no animation) and sets it as `MuiTextField`'s default. So under the preset every field is `variant="stacked"`, and `variant="outlined"` on one field opts that field back to MUI's floating label. Under a stock `createTheme()` `'stacked'` typechecks and renders, but it is an outlined box with a floating label and no notch — the look is the preset's, not the type's.

**Known limitation.** The preset's _box_ rules (root border, focus ring, padding) stay theme-wide in this pass, so under the preset `variant="outlined"` floats its label over the root's solid border. A follow-up ("preset: a real notched look for `variant="outlined"`") keys the box rules on the variant.
