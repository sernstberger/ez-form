...
**Floating is `InputLabel`'s, not this axis's.** Whether a `top` label sits over the input and translates up on focus is MUI's `InputLabel` `shrink` (with `disableAnimation` and `OutlinedInput` `notched`), which is a theme concern in vanilla MUI and stays one here. So under stock `createTheme()` a `top` label floats, and under `createEzFormTheme()` — which shrinks `MuiInputLabel` and un-notches `MuiOutlinedInput` — the same `top` label is static, above the control in normal flow, with helper text flush left. That is what `DESIGN.md` calls for. A consumer on a stock theme who wants static labels writes the same three theme lines a vanilla MUI consumer writes today.
...
place that holds an opinion — static labels (`MuiInputLabel` permanently shrunk, so a `top`
label sits above the input with no floating animation and no notch), flat bordered inputs
